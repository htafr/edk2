  ...
  )
{
  ...
  do {
    ...
    // Read response
    Count  = 2 * 4;
    Status = PciIoDoeReceive (This, &Count, &Response);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "%a: Probing PCIe device to discover DOE capability\n", __func__));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "%a: PCIe DOE not supported\n", __func__));
    return EFI_UNSUPPORTED;
  }
  ...
  Status = PciIoDoeTypeCheck (&PciIoDevice->PciDoe, PCI_VENDOR_ID_PCI_SIG, PCI_DOE_PROTOCOL_DISCOVERY);
  if (EFI_ERROR (Status)) {
