...
typedef struct {
  ...
  ///   is PciIo. EDKII_DEVICE_IDENTIFIER_TYPE_USB_GUID means the device access
  ///   protocol is UsbIo.
  ///
  EFI_GUID DeviceType;
  ///
  ...
  /// device handle. 1) An EFI_DEVICE_PATH_PROTOCOL with
  /// EFI_DEVICE_PATH_PROTOCOL_GUID. 2) A device access protocol with
  ...
  ///
  ///    The device access protocol is required, because the verifier need have
  ///    a way to communciate with the device hardware to get the measurement or
  ///    do the challenge/response for the device authentication.
  ///
  ...
  ///       driver register a protocol notify function. Installing a real
  ///       protocol may cause the callback function being executed before the
...
EFI_DEVICE_PATH_PROTOCOL_GUID, and the device access protocol with
EDKII_DEVICE_IDENTIFIER_TYPE_xxx_GUID. Once it is done, a DeviceHandle is
returned. 5) Device driver creates EDKII_DEVICE_IDENTIFIER with
EDKII_DEVICE_IDENTIFIER_TYPE_xxx_GUID and the DeviceHandle. 6) Device driver
calls DeviceAuthenticate(). 7) If DeviceAuthenticate() returns
EFI_SECURITY_VIOLATION, the device driver uninstalls all protocols on this
handle. 8) If DeviceAuthenticate() returns EFI_SUCCESS, the device driver
installs the device access protocol with a real protocol GUID. e.g.
...
the measurement and/or authentication based upon the platform policy. If TCG
measurement is required, the measurement is extended to TPM PCR.
  @retval EFI_SECURITY_VIOLATION   The device fails to return the measurement
data.
  @retval EFI_SECURITY_VIOLATION   The device fails to response the
authentication request.
  @retval EFI_SECURITY_VIOLATION   The system fails to verify the device based
upon the authentication response.
  @retval EFI_SECURITY_VIOLATION   The system fails to extend the measurement to
TPM PCR.
**/
typedef EFI_STATUS(EFIAPI *EDKII_DEVICE_AUTHENTICATE)(
    IN EDKII_DEVICE_SECURITY_PPI *This, IN EDKII_DEVICE_IDENTIFIER *DeviceId);
