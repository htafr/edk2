    ...
    uint8_t *signature, size_t *sig_size)
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "libspdm_requester_data_sign: PeiServicesLocatePpi - %r\n", Status));
    return FALSE;
  }

  PrivateKeySize = 4096; // RSA2048
  PrivateKey = AllocateZeroPool(PrivateKeySize);
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "libspdm_requester_data_sign: GetVariable - %r\n", Status));
    return FALSE;
  }

  // DEBUG ((DEBUG_INFO, "%a:\n\t", __func__));
  // for (UINTN Index = 0; Index < PrivateKeySize; Index++) {
  //   if ((Index != 0) && (Index % 32 == 0)) DEBUG ((DEBUG_INFO, "\n\t"));
  //   DEBUG ((DEBUG_INFO, "%02X ", PrivateKey[Index]));
  // }
  // DEBUG ((DEBUG_INFO, "\n"));
...
//                                                            &context);
  Result = libspdm_rsa_get_private_key_from_pem (
              PrivateKey,
              PrivateKeySize,
              NULL,
              &Context
  ...
  if (!Result) {
    DEBUG ((DEBUG_ERROR, "libspdm_requester_data_sign: libspdm_rsa_get_private_key_from_pem - ERROR\n"));
    return FALSE;
  }
